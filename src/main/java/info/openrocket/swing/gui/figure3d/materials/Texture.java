...
public class Texture {
	 ...
	 * @param filePath The path to the texture file (e.g., "src/main/resources/textures/decal.png").
	 */
	public Texture(String filePath) {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			System.err.println("Exception while loading texture: " + e.getMessage());
		}
	}
	...
	public Texture(String[] cubemapFiles) {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			for (int i = 0; i < cubemapFiles.length; i++) {
				ByteBuffer image = STBImage.stbi_load(cubemapFiles[i], w, h, channels, 0);
				if (image == null) {
					throw new RuntimeException("Failed to load texture file: " + cubemapFiles[i] + "\n" + STBImage.stbi_failure_reason());
				}
	...
	public Texture(String atlasFilePath, AtlasLayout layout) {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			ByteBuffer atlasBuffer = STBImage.stbi_load(atlasFilePath, w, h, channels, 0);
			if (atlasBuffer == null) {
				throw new RuntimeException("Failed to load atlas file: " + atlasFilePath + "\n" + STBImage.stbi_failure_reason());
			}
	...
	public Texture(String filePath, boolean isHDR) {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			FloatBuffer image = STBImage.stbi_loadf(filePath, w, h, channels, 0);
			if (image == null) {
				throw new RuntimeException("Failed to load HDR image: " + filePath + "\n" + STBImage.stbi_failure_reason());
			}
