...
public class Standalone3DEngine implements Runnable {
	...
	private void loop() {
        ...
        glViewport(0, 0, fbSize[0], fbSize[1]);
				...
				scene3DOrchestrator.getRenderer().render(scene3DOrchestrator.getScene(), windowManager, !scene3DOrchestrator.isExportTransparent());

				// Swap to make it visible
				windowManager.swapBuffers();

				// Export the visible front buffer
				try {
					String filePath = "export_" + System.currentTimeMillis() + ".png";
					ImageExporter exporter = new PngExporter();
					exporter.export(fbSize[0], fbSize[1], filePath);
				} catch (IOException e) {
