...
public class DemoFactory {
	...
	public static Rocket createTestRocket() {
		...
		finSet2Appearance = new Appearance(finSet2Appearance.getPaint(), finSet2Appearance.getShine(),
				new Decal(
						...
						new Coordinate(1, 1),
						0,
						new FileDecalImage(new File("src/main/resources/textures/test_texture.png")), Decal.EdgeMode.REPEAT));
	...
	public static void setupTerrain(Scene scene) {
		...
		/*try {
			...
			System.err.println("Could not create terrain from GPS: " + e.getMessage());
			e.printStackTrace();
		}*/
		...
		try {
			...
			String customFloorImagePath = "src/main/resources/textures/grass.jpg";
			float size = 200.0f;
			scene.addObject(terrainGenerator.createTerrainFromFile(customFloorImagePath, size, 20.0f));
		} catch (Exception e) {
			System.err.println("Could not create terrain from file: " + e.getMessage());
			e.printStackTrace();
		}
	}
		...
		switch (option) {
			...
			case 2:
				try {
					...
					System.err.println("Could not load skybox, falling back to gradient: " + e.getMessage());
					scene.setBackground(new GradientBackground(
							new Vector3f(0.8f, 0.9f, 1.0f), // Light blue top
							new Vector3f(0.2f, 0.3f, 0.5f)  // Dark blue bottom
					));
			...
			case 3:
				try {
					String atlasPath = "src/main/resources/textures/backgrounds/skybox_atlas_horizontal.png";
					scene.setBackground(new SkyboxBackground(new Texture(atlasPath, Texture.AtlasLayout.HORIZONTAL_CROSS)));
				} catch (Exception e) {
					System.err.println("Could not load skybox atlas, falling back to gradient: " + e.getMessage());
					scene.setBackground(new GradientBackground(
							new Vector3f(0.8f, 0.9f, 1.0f), // Light blue top
							new Vector3f(0.2f, 0.3f, 0.5f)  // Dark blue bottom
					));
			...
			case 4:
				try {
					Texture hdriTexture = new Texture("src/main/resources/textures/backgrounds/hdri_sky.hdr", true);
					scene.setBackground(new HDRIBackground(hdriTexture));
				} catch (Exception e) {
					System.err.println("Could not load HDRI background, falling back to gradient: " + e.getMessage());
					scene.setBackground(new GradientBackground(
							new Vector3f(0.8f, 0.9f, 1.0f), // Light blue top
							new Vector3f(0.2f, 0.3f, 0.5f)  // Dark blue bottom
					));
