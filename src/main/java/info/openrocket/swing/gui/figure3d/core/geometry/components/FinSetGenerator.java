...
public class FinSetGenerator {
	...
	public static Mesh create(FinSet finSet, SymmetricComponent parent, RenderingConfiguration config) {
		Coordinate[] finShape = finSet.generateContinuousFinAndTabShape();
		...
		List<Coordinate> combinedShape = new ArrayList<>();
		if (finShape != null) {
		...
		for (Coordinate c : combinedShape) {
	...
	public static Mesh createFilletMesh(FinSet finSet, SymmetricComponent parent,
		...
		Coordinate[] shapePoints = finSet.generateContinuousFinAndTabShape();

		float axialOffset   = (float) finSet.getAxialOffset(AxialMethod.TOP);
		float rootChordStart= (float) rootPoints[0].x;
		float rootChordEnd  = (float) rootPoints[rootPoints.length - 1].x;
			...
			for (int j = 0; j <= filletSegments; j++) {
				...
				if (j < filletSegments) {
					...
					for (int k = 0; k < shapePoints.length; k++) {
						Coordinate a = shapePoints[k];
						Coordinate b = shapePoints[(k + 1) % shapePoints.length];
					...
					if (spanOK) {
						...
						// DEBUG (good to keep for now)
						if (j == 0 && (currentX != clampedX)) {
							System.out.println("[fillet] clamp i=" + i + " y=" + yRow +
									" x: " + currentX + " -> " + clampedX +
									"  span=[" + xMin + "," + xMax + "]");
						}
					}
	...
	private static void addFilletCaps(List<Vertex> vertices, List<Integer> indices, int[][] gridIndices, int segmentIndex, Coordinate rootPoint, FinSet finSet, SymmetricComponent parent, int arcSegments, boolean isEndCap) {
		float x = (float) rootPoint.x;
		float y = (float) rootPoint.y; // y-coordinate of the fin root from the 2D profile
		float axialOffset = (float) finSet.getAxialOffset(AxialMethod.TOP);
	 ...
	 */
	private static boolean isRootPoint(Coordinate p, Coordinate[] rootPoints, float tolerance) {
		if (rootPoints == null) return false;
		for (Coordinate rootPoint : rootPoints) {
			if (Math.abs(p.x - rootPoint.x) < tolerance && Math.abs(p.y - rootPoint.y) < tolerance) {
				return true;
			}
		}
		return false;
	...
	private static FinMeshData createShapeMesh(Coordinate[] shapePoints, FinSet finSet,
		...
		for (Coordinate c : shapePoints) {
			float u = ((float) c.x - globalMinX) / globalSpanX;
			float v = ((float) c.y - globalMinY) / globalSpanY;
			meshData.vertices.add(new Vertex(new Vector3f((float) c.x, (float) c.y, (float) finSet.getThickness() / 2), new Vector3f(0, 0, 1), new Vector2f(u, v), 0));
		}
		for (Coordinate c : shapePoints) {
			float u = ((float) c.x - globalMinX) / globalSpanX;
			float v = ((float) c.y - globalMinY) / globalSpanY;
		...
		for (int i = 0; i < shapePoints.length; i++) {
			Coordinate p1 = shapePoints[i];
			Coordinate p2 = shapePoints[(i + 1) % shapePoints.length];

			if (new Vector3f((float)p1.x, (float)p1.y, 0).equals(new Vector3f((float)p2.x, (float)p2.y, 0), EPSILON)) {
				segmentTypes[i] = SegmentType.DEGENERATE;
				segmentLengths[i] = 0;
				continue;
			}
			segmentLengths[i] = (float) Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2));
		...
		for (int i = 0; i < shapePoints.length; i++) {
			...
			if (type == SegmentType.ROOT) {
				...
				if (finSet instanceof EllipticalFinSet) {
					...
					Coordinate p1 = shapePoints[p1_idx];
					Coordinate p2 = shapePoints[p2_idx];

					// Get coordinates relative to the ellipse's center to calculate the gradient
					double p1_x_rel = p1.x - a;
					double p2_x_rel = p2.x - a;

					// The normal is derived from the gradient of (x/a)^2 + (y/b)^2 = 1
					Vector3f normal1 = new Vector3f((float)(p1_x_rel / (a * a)), (float)(p1.y / (b * b)), 0).normalize();
					Vector3f normal2 = new Vector3f((float)(p2_x_rel / (a * a)), (float)(p2.y / (b * b)), 0).normalize();
	...
	private static void createRootConnectionFaces(FinMeshData meshData, Map<Integer, CurvedRootInfo> curvedRootInfoMap,
												  Map<Integer, SideEdgeInfo> sideEdgeInfoMap, Coordinate[] shapePoints,
	...
	private static List<Integer> triangulateWithJTS(Coordinate[] polygonPoints) {
		Map<String, Integer> coordIndexMap = new HashMap<>();
		for (int i = 0; i < polygonPoints.length; i++) {
			coordIndexMap.put(polygonPoints[i].x + "," + polygonPoints[i].y, i);
		}

		org.locationtech.jts.geom.Coordinate[] jtsCoords = new org.locationtech.jts.geom.Coordinate[polygonPoints.length + 1];
		for (int i = 0; i < polygonPoints.length; i++) {
			jtsCoords[i] = new org.locationtech.jts.geom.Coordinate(polygonPoints[i].x, polygonPoints[i].y);
		}
		jtsCoords[polygonPoints.length] = new org.locationtech.jts.geom.Coordinate(polygonPoints[0].x, polygonPoints[0].y);
