...
public abstract class RocketMeshBuilder {
	...
	private static void buildComponent(SceneView scene, FlightConfigurationId fcid, RocketComponent component,
		...
		Coordinate[] locations = component.getComponentLocations();
		Coordinate[] angles = component.getComponentAngles();
		...
		for (int i = 0; i < locations.length; i++) {
			...
			Coordinate instanceLocation = locations[i];
			Coordinate instanceAngle = angles[i].add(angleOffsetX, 0, 0);
	...
	private static <T extends RocketComponent & MotorMount> void buildMotor(
			SceneView scene, FlightConfigurationId fcid, T mount, Coordinate mountLocation, Coordinate mountAngle,
		...
		Coordinate mountFrontAbsolute = mount.toAbsolute(new Coordinate(0,0,0))[0];
		Coordinate motorCenterAbsolute = mountFrontAbsolute.add(motorCenterRelToMountFront, 0, 0);
		...
		Vector3f positionInEngineCS = new Vector3f(
				(float) motorCenterAbsolute.x,
				(float) motorCenterAbsolute.y,
				(float) motorCenterAbsolute.z * -1.0f
		).mul(worldScale);

		Matrix4f rotationMatrix = new Matrix4f()
				.rotateX(-(float) mountAngle.x)
				.rotateY(-(float) mountAngle.y)
				.rotateZ(-(float) mountAngle.z);
	 ...
	 */
	private static void configureComponentTransform(SceneObject obj, Coordinate loc, Coordinate ang, RocketComponent component, float worldScale) {
		...
		Vector3f positionInEngineCS = new Vector3f(
				(float)(loc.x + offsetX),
				(float)(loc.y),
				(float)(loc.z) * -1.0f // Flip Z to convert from LHS to RHS
		).mul(worldScale);
	...
	private static <T extends RocketComponent & MotorMount> void rebuildMotorParticles(
		...
		Coordinate[] locations = mount.getComponentLocations();
		Coordinate[] angles = mount.getComponentAngles();
		...
		for (int i = 0; i < locations.length; i++) {
			Coordinate instanceLocation = locations[i];
			Coordinate instanceAngle = angles[i];
			...
			Matrix4f rotationMatrix = new Matrix4f()
					.rotateX(-(float) instanceAngle.x)
					.rotateY(-(float) instanceAngle.y)
					.rotateZ(-(float) instanceAngle.z);
