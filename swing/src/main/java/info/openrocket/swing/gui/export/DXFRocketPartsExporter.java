...
import info.openrocket.core.file.dxf.export.FinDxfExporter;
import info.openrocket.core.file.dxf.export.ProfileDxfExporter;
import info.openrocket.core.file.dxf.export.RailButtonDxfExporter;
import info.openrocket.core.file.dxf.export.TubeDxfExporter;
...
public class DXFRocketPartsExporter {
	...
	public void export(OpenRocketDocument document, File destination, DXFExportOptions options)
			throws IOException {
	...
	public void export(List<RocketComponent> components, File destination, DXFExportOptions options)
			throws IOException {
	...
	private double calculateRowWidth(List<Part> parts, DXFExportOptions options) {
	...
	private void layoutParts(DXFBuilder builder, List<Part> parts, double rowWidth, DXFExportOptions options) {
		double cursorX = 0;
		double cursorY = 0;
		double rowHeight = 0;
		...
		for (Part part : parts) {
			...
			if (options.isShowLabels() && part.label != null && !part.label.trim().isEmpty()) {
				...
				builder.addText(labelX, labelY, part.label, LABEL_FONT_SIZE_MM, options.getLabelColor(), "LABELS");
			}
	...
	private interface PartRenderer {
		void render(DXFBuilder builder, double originX, double originY, DXFExportOptions options);
	}
	...
	public static final class Part {
		...
		static Part fromFinSet(FinSet finSet) {
			...
			return new Part(width, height, (builder, originX, originY, options) -> {
				...
				FinDxfExporter.drawFinSet(finSet, builder, offsetX, offsetY, options);
			}, label);
		}

		static Part fromProfile(SymmetricComponent comp) {
			ProfileDxfExporter.Bounds b = ProfileDxfExporter.calculateBounds(comp);
			...
			return new Part(width, height, (builder, originX, originY, options) -> {
				...
				ProfileDxfExporter.drawClosedProfile(comp, builder, originX, midY, options);
			}, label);
		}

		static Part fromTube(Coaxial tube) {
			double length = ((RocketComponent) tube).getLength();
			TubeDxfExporter.Bounds bounds = TubeDxfExporter.calculateBounds(tube, length);
			double width = bounds.getWidth();
			double height = bounds.getHeight();
			...
			return new Part(width, height, (builder, originX, originY, options) -> {
				...
				TubeDxfExporter.drawTubeProfile(tube, length, builder, originX, centerY, options);
			}, label);
		}
		...
		static Part fromRailButton(RailButton railButton) {
			...
			return new Part(width, height, (builder, originX, originY, options) -> {
				...
				RailButtonDxfExporter.drawRailButtonProfile(railButton, builder, adjustedX, adjustedY, options);
			}, label);
		}
	}
}
