	...
	private static Appearance3D orAppearanceToAppearance3D(Appearance orAppearance, ExternalComponent.Finish finish) {
		...
		if (orDecal != null && orDecal.getImage() != null) {
			try (InputStream is = orDecal.getImage().getBytes()) {
				...
				transform.scale.set((float) orDecal.getScale().x, (float) orDecal.getScale().y);
				transform.offset.set((float) orDecal.getOffset().x, (float) orDecal.getOffset().y);
				transform.rotation = (float) orDecal.getRotation();
			} catch (Exception e) {
				System.err.println("Failed to load decal image from OpenRocket component.");
				e.printStackTrace();
			}
		}
