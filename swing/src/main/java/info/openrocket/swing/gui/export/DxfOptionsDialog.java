...
import info.openrocket.swing.gui.components.DXFOptionPanel;
...
/**
 * Modal dialog wrapping the DXFOptionPanel with component selection tree.
 */
public class DxfOptionsDialog extends JDialog {
	public static final int COMPONENTS_TAB = 0;

	private static final Translator trans = Application.getTranslator();
	private final DXFOptionPanel optionsPanel;
	...
	public DxfOptionsDialog(Frame owner, OpenRocketDocument document, List<RocketComponent> initiallySelectedComponents) {
		super(owner, "DXF Export Options", true);
		this.document = document;
		optionsPanel = new DXFOptionPanel(true);
		
		// Get all exportable components (reuse SVG exporter's collection method)
		exportableComponents = DXFRocketPartsExporter.collectExportableComponents(document);
	...
	private void initialize() {
		...
		// Create tabbed pane with Components tab
	...
	public DXFExportOptions getExportOptions() {
		double partSpacing = optionsPanel.getPartSpacing();
		
		return new DXFExportOptions(
			getStrokeColor(),
			getStrokeWidth(),
			isDrawCrosshair(),
			getCrosshairColor(),
	...
	public void setFromPreferences(ApplicationPreferences prefs) {
		...
		// Crosshair size is loaded in DXFOptionPanel constructor
	}

	public DXFOptionPanel getOptionsPanel() {
		return optionsPanel;
	}
	 ...
	 * @return The selected tab index (COMPONENTS_TAB)
	 */
