...
/**
 * Utilities to render tube components (BodyTube, LaunchLug) with both side and back profiles to DXF.
 ...
 */
public final class TubeDxfExporter {
	...
	public static void drawTubeProfile(Coaxial component, double length,
		...
		builder.addPath(rectangle, options.getStrokeColor(), "PROFILES");
		...
		// Draw outer circle
		builder.addCircle(backCenterX, backCenterY, outerRadius, options.getStrokeColor(), "PROFILES");
		
		// Draw inner circle if not filled and inner radius > 0
		if (!filled && innerRadius > 0) {
			builder.addCircle(backCenterX, backCenterY, innerRadius, options.getStrokeColor(), "PROFILES");
		}
		...
		if (options.isDrawCrosshair()) {
			...
			builder.addCrosshair(backCenterX, backCenterY, halfLength, halfLength,
					options.getCrosshairColor(), "CROSSHAIRS");
		}
	}
