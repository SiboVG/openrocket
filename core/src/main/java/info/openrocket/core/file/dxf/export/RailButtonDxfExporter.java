...
/**
 * Utilities to render side profiles of rail buttons as 3 stacked rectangles to DXF.
 */
public final class RailButtonDxfExporter {
	private RailButtonDxfExporter() {}
	 ...
	 */
	public static void drawRailButtonProfile(RailButton railButton, DXFBuilder builder,
		...
		if (baseHeight > 0) {
			...
			builder.addPath(basePath, options.getStrokeColor(), "PROFILES");
		}
		...
		if (innerHeight > 0) {
			...
			builder.addPath(innerPath, options.getStrokeColor(), "PROFILES");
		}
		...
		if (flangeHeight > 0) {
			...
			builder.addPath(flangePath, options.getStrokeColor(), "PROFILES");
		}
