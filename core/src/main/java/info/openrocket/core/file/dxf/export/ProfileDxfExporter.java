...
/**
 * Utilities to render side profiles of symmetric components (nose cones, body tubes, transitions) to DXF.
 * Similar to ProfileSvgExporter but for DXF format.
 */
public final class ProfileDxfExporter {
	private ProfileDxfExporter() {}
	 ...
	 */
	public static void drawClosedProfile(SymmetricComponent component, DXFBuilder builder,
	                                     double originX, double originY, DXFExportOptions options) {
		builder.addPath(sampleClosed(component, originX, originY), options.getStrokeColor(), "PROFILES");
	 ...
	 */
	private static void drawShoulders(Transition transition, DXFBuilder builder,
	                                  double originX, double originY, DXFExportOptions options) {
		double transitionLength = transition.getLength();
		...
		if (transition.getForeShoulderLength() > 0) {
			...
			builder.addPath(foreShoulderPath, options.getStrokeColor(), "PROFILES");
		}
		...
		if (transition.getAftShoulderLength() > 0) {
			...
			builder.addPath(aftShoulderPath, options.getStrokeColor(), "PROFILES");
		}
	}
